#!/bin/bash
# Internal Chroma Initialization Script for RPGer
# This script initializes the Chroma vector database from within the backend container
...
echo -e "${BLUE}=========================================${NC}"
echo -e "${BLUE}   RPGer Internal Chroma Initialization ${NC}"
echo -e "${BLUE}=========================================${NC}"
...
docker cp "$PYTHON_SCRIPT" "$BACKEND_CONTAINER:/app/init_chroma.py"
...
docker exec "$BACKEND_CONTAINER" python /app/init_chroma.py --host chroma --port 8000 $VERBOSE
...
# Clean up
echo -e "\n${BLUE}Cleaning up...${NC}"
docker exec "$BACKEND_CONTAINER" rm -f /app/init_chroma.py
